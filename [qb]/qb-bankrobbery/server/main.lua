...
local timeOutInMinutesSmallBanks = 5 -- standard 30 minutes
local timeOutInMinutesBigBanks = 5 -- standard 90 minutes
...
AddEventHandler('qb-bankrobbery:server:setTimeout', function()
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                Citizen.Wait(timeOutInMinutesBigBanks * (60 * 1000))
                timeOut = false
                robberyBusy = false
...
AddEventHandler('qb-bankrobbery:server:SetSmallbankTimeout', function(BankId)
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                Citizen.Wait(timeOutInMinutesSmallBanks * (60 * 1000))
                timeOut = false
                robberyBusy = false
